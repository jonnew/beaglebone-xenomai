...
#define XENOMAI_SECONDARY_INTERRUPT 118	// IRQ 118 is unused on AM335x
#define INTCPS_ISR_CLR0				0x94

unsigned int secondary_edma_irq[2];
EXPORT_SYMBOL(secondary_edma_irq);
...
/******************************************************************************
 *
 * XENOMAI secondary interrupt handler
 *
 *****************************************************************************/
static irqreturn_t xenomai_irq_handler(int irq, void *data)
{
	int i;
	unsigned ctlr;
	unsigned int cnt = 0;
	...
	dev_dbg(data, "xenomai_irq_handler\n");

	if ((secondary_edma_irq[0] == 0)
	    && (secondary_edma_irq[1] == 0))
		return IRQ_NONE;
		
	while (1) {
		int j;
		if (secondary_edma_irq[0])
			j = 0;
		else if (secondary_edma_irq[1])
			j = 1;
		else
			break;
		for (i = 0; i < 32; i++) {
			int k = (j << 5) + i;
			if (secondary_edma_irq[j] &
							(1 << i)) {
				/* Clear the corresponding IPR bits */
				secondary_edma_irq[j] &= ~(1 << i);
				if (edma_cc[ctlr]->intr_data[k].callback) {
					edma_cc[ctlr]->intr_data[k].callback(
						k, DMA_COMPLETE,
						edma_cc[ctlr]->intr_data[k].
	...
	// reset soft IRQ
	__raw_writel(1<<(XENOMAI_SECONDARY_INTERRUPT%32), intc_base + INTCPS_ISR_CLR0 + 0x20*(XENOMAI_SECONDARY_INTERRUPT/32));
...
#if defined(CONFIG_SOC_OMAPAM33XX)
		intc_base = ioremap( TI81XX_ARM_INTC_BASE, SZ_2K );
#endif
...
#if defined(CONFIG_SOC_OMAPAM33XX)
		status = request_irq(XENOMAI_SECONDARY_INTERRUPT, xenomai_irq_handler, 0, "edma",
					&pdev->dev);
		if (status < 0) {
			dev_dbg(&pdev->dev, "request_irq %d failed --> %d\n",
				XENOMAI_SECONDARY_INTERRUPT, status);
			goto fail1;
		}
#endif
...
#if defined(CONFIG_SOC_OMAPAM33XX)
	free_irq(XENOMAI_SECONDARY_INTERRUPT, &pdev->dev);
#endif
